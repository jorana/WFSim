...
class Resource(object):
    ...
    class __Resource(object):
        ...
        def __init__(self, config={}):
            ...
            # Pulse
            self.photon_area_distribution = get_resource('/Users/petergaemers/Desktop/python/WFSim/notebooks/spe_dist.csv', fmt='csv')
            ...
            if config['detector'] == 'XENONnT':
                self.s1_pattern_map = itp_map('/Users/petergaemers/Downloads/XENONnT_s1_xyz_patterns_corrected_MCv3.0.0.json.gz', fmt='json.gz')
            ...
            if config['detector'] == 'XENONnT':
                self.s2_light_yield =  get_resource('/Users/petergaemers/Downloads/XENONnT_s2_xy_patterns_topbottom_corrected_MCv3.0.0.json.gz',fmt = 'json.gz')
                self.s2_light_yield['map'] = np.sum(self.s2_light_yield['map'][:][:],axis=2)
                self.s2_light_yield_map = itp_map(self.s2_light_yield)

            if config['detector']=='XENON1T':
                self.s2_per_pmt_params = get_resource(self.config['s2_pattern_map'], fmt='json.gz')
            if config['detector'] == 'XENONnT':
                self.s2_pattern_map = itp_map('/Users/petergaemers/Downloads/XENONnT_s2_xy_patterns_topbottom_corrected_MCv3.0.0.json.gz', fmt='json.gz')
