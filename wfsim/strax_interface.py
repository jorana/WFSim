...
def rand_instructions(c):
    ...
    r = np.sqrt(np.random.uniform(0, 66.5**2, n))
    t = np.random.uniform(-np.pi, np.pi, n)
    instructions['x'] = np.repeat(r * np.cos(t), 2)
    instructions['y'] = np.repeat(r * np.sin(t), 2)
    instructions['z'] = np.repeat(np.random.uniform(-10, 0, n), 2)

    nphotons = np.repeat(100, n)
    nelectrons = np.repeat(130,n)
...
@strax.takes_config(
    ...
    strax.Option('fax_config', 
                 default='/Users/petergaemers/Desktop/python/WFsim/notebooks/fax_config_nt.json'),
...
#                      'strax_auxiliary_files/master/fax_files/fax_config_NV.json'),
#     strax.Option('detector', default = 'XENONnT_NV'
#                  ),
# )
# class RawRecordsNvFromFax(FaxSimulatorPlugin):
